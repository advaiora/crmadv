...
> **⚠️ DA DECIDERE — una chiamata AI che fallisce dopo aver speso non entra nel budget giornaliero (rilievo del 🛡️ Guardiano al cancello di CRMA-55, 10/9/2026; non è un difetto di CRMA-55, che ha riprodotto fedelmente il comportamento già esistente).**
>
> **Il fatto, verificato sul commit `3338d2e`** (ramo `backend/crma-55-budget-e-log-ricerca-competitor`, non ancora unito a `main`; `server/modules/agency-os/agency.service.ts`): tre scritture di `AiUsageLog` stanno **dopo** i punti in cui la funzione può lanciare, e scrivono solo `status: 'success'` — motore JSON (`:3357`, dopo il `throw` di `:3345`), motore testo (`:3570`, dopo il `throw` di `:3555`), ricerca competitor (`:3948`, aggiunta da CRMA-55, dopo il parse dello schema). **C'è una quarta scrittura con lo stesso difetto, fuori da questo file:** l'embedder delle fonti in `server/modules/sources/sources.rag.ts:158` scrive `status: 'success'` dopo il `throw` di `sources.rag.ts:88`, e se `promptTokens <= 0` non scrive nemmeno quella. Il fusibile `assertWithinAiBudget` somma `AiUsageLog`: quello che non è scritto lì, per il budget non è mai costato niente. Chi ha il permesso `ai_production.generate` e incappa in un percorso che fallisce in modo ripetibile può quindi spendere senza limite, senza bisogno di malafede — basta un modello che smette di rispettare il formato atteso.
>
> **La ricerca competitor è il caso peggiore dei tre**, pur essendo il più recente: paga il `web_search` **a chiamata oltre ai token**, il ramo Anthropic può aver fatto **fino a quattro giri fatturati** prima del parse che fallisce, il suo timeout (`getCompetitorSearchTimeoutMs()`) aborta una richiesta che il provider ha già eseguito e fatturato, e il parse è quello con **più probabilità di fallire tardi** (JSON strutturato ricavato da una ricerca web in linguaggio libero, più fragile di una generazione normale).
>
> **Le due strade, non ancora scelte:**
> - **A — scrivere anche le righe fallite**, con `status: 'failed'` (il valore coerente col vocabolario già in uso, `'success' | 'cache_hit' | 'failed'` in `agency.service.ts:2234`; non `'error'`, che introdurrebbe un quarto valore) e i token letti fin dove si è arrivati. Il campo `status` in `AiUsageLog` è una stringa libera in schema (`prisma/schema.prisma:878`, `@db.VarChar(30)`), quindi **non serve nessuna migrazione**. **Verificato:** `sumCostForUser` (`server/repositories/ai-usage.repository.ts:134-140`) **non filtra per `status`** — le righe fallite entrerebbero nei totali del rendiconto "Consumi & costi AI" senza preavviso, va deciso se è il comportamento voluto o se il rendiconto va aggiornato insieme. Per la ricerca competitor la variante minima è **tre righe**: spostare la chiamata a `logCompetitorSearchUsage` da dopo il `parse` a subito dopo la lettura di `usage`, che chiude il caso più caro dei tre senza toccare gli altri tre (i due motori e l'embedder delle fonti).
> - **B — lasciare com'è e dirlo**, accettando che il budget sia una stima per difetto. Costa zero e va scritto nella pagina dei consumi, altrimenti il numero sembra esatto e non lo è.
>
> Scelta di Jacopo o Claudio, non decisa qui.
...
- 🔴 **Il cambio password è backend senza interfaccia — e la roadmap qui sotto lo dà per un blocco ancora aperto, correttamente** *(trovato il 9/9/2026 unendo i nove rami della release, confermato dal revisore)*. Il ramo `crm-31` ha portato `POST /auth/password/change` (`server/modules/password/routes/password.route.ts`, registrata in `server/app.ts`), **ma nessuna chiamata del frontend ci punta**: non esiste `src/modules/password/`, e `/reset-password` è la pagina del tema (62 righe, zero chiamate API, già su `main` e già in `RouteList.jsx`). Non è un difetto dell'unione — `crm-31` è consapevolmente solo-backend — ma **va dichiarato nelle note di release**, altrimenti chi legge il changelog conclude che il cambio password è arrivato mentre nel CRM non c'è nessun punto da cui farlo. È il tipo di errore che si scopre in produzione, dal cliente. Il seguito naturale è la pagina **Impostazioni Account**.

- **La tabella `PasswordResetToken` è in produzione senza nessun lettore né scrittore** *(9/9/2026)*. Migrazione `20260831170847`, modello a `prisma/schema.prisma:313`; `grep` su `server/`, `src/` e `scripts/`: **zero occorrenze**. Nulla di rotto oggi, ma la tabella suggerisce una funzione che non c'è. ⚠️ **Da sapere prima di aprire il ramo del recupero password:** `password.service.ts` dichiara che le sessioni aperte **non** vengono revocate al cambio password, perché servirebbe un `passwordChangedAt` su `User` — quindi completare il recupero porterà **una terza migrazione**. Per la regola «una migrazione non sta mai su un ramo lungo», va messa in conto **prima** di cominciare, non scoperta a metà.

- **`npm run security:rbac-catalog` esiste ma non lo lancia niente** *(9/9/2026)*. Lo script è arrivato con `crm-35` (`package.json`), ma il `.githooks/pre-commit` rigenera solo la mappa e non esiste nessun `.github/workflows/`. **Un controllo che nessuno esegue è un controllo che non esiste**: il permesso fuori catalogo tornerebbe a passare, cioè esattamente ciò che `crm-35` è stato scritto per impedire. Oggi passa (188 file, 73 permessi, nessuna chiave sconosciuta), ma per abitudine di chi se lo ricorda. **Da agganciare** al pre-commit o a `test:backend` — è una riga, il valore è che smette di dipendere dalla memoria.

- **Il guardiano di rete è diventato più severo per *tutti* i chiamanti, non solo per la posta** *(9/9/2026, da tenere d'occhio dopo la messa online)*. `crm-28` ha aggiunto a `server/core/net-guard.ts` la fascia CGNAT `100.64.0.0/10` e ha riscritto il controllo IPv6 da confronto di prefissi a espansione completa. Ma `isPrivateIpv4Address`/`isPrivateIpv6Address` servono anche `safeFetch` e `assertPublicHttpUrl`, **usati fuori dalla posta**: un sito in gestione raggiunto attraverso una VPN su quella fascia comincerebbe a rispondere «host privato» senza che nessuno abbia toccato quel modulo. Il file lo dichiara già (commento sul caso Tailscale), quindi è una scelta consapevole e non una svista — **è una cosa da guardare nei primi giorni dopo il lancio**, non un difetto da correggere adesso.

- **`src/views/Settings/MailServer.jsx` è a 494 righe, tredici sotto la soglia** *(9/9/2026)*. Era 457; le tre rifiniture della posta l'hanno portata lì. **La prossima aggiunta lo manda oltre le 500**: chi tocca quella maschera estrae qualcosa prima di aggiungere, come dice la regola. Segnalato qui perché è esattamente il caso di `views/Team/index.jsx`, cresciuto invece di essere spezzato.
...
- **Gli elenchi `select` dei repository possono perdere un campo senza che nessuno se ne accorga** *(trovato l'1/9/2026 aggiungendo una colonna al modulo «Server di posta», CRM-28)*. I repository chiedono a Prisma un elenco **esplicito** di colonne. Un campo aggiunto allo schema e al tipo del record, ma dimenticato in quell'elenco, si **salva e non si rilegge**: nessun errore, nessun test rosso, e il guasto si vede solo ricaricando la maschera. Non c'è nessuna rete che lo prenda — il progetto non ha uno script di typecheck (`tsc --noEmit` non è in `package.json`), `tsx` non controlla i tipi, ESLint non è *type-aware*, e nel checkout condiviso i tipi del client Prisma si risolvono comunque a `any`. **Misura: 18 file `*.repository.ts` con elenchi `select` espliciti** (i più carichi: `agency-os/agency.repository.ts` 48 blocchi, `dashboard` 29, `checklists` 21); il rischio cresce con i campi, non con i blocchi. **Come si è chiuso nel modulo posta, se si vuole imitarlo:** i due `select` identici sono diventati una costante sola (`CAMPI_LETTI` in `server/modules/mail/mail.repository.ts`) e un test scorre le chiavi di una riga completa verificando che ognuna sia chiesta al database (`mail.repository.test.ts`). Costa cinque minuti per repository e non richiede nessuna dipendenza nuova. **Da fare quando si passa su un repository, non in una passata unica** — e comunque prima o insieme all'eventuale azzeramento della baseline TypeScript qui sopra, che risolverebbe lo stesso problema in modo più generale ma è un lavoro molto più grosso.

- **Etichette dei form non collegate ai campi (accessibilità)** *(censito il 4/8/2026)*. Nel frontend le `<Form.Group>` di react-bootstrap che contengono una `<Form.Label>` spesso **non hanno `controlId`**: senza, la label non riceve `htmlFor` e il campo non riceve `id`, quindi non risultano collegati. Chi usa uno screen reader non sente a cosa serve il campo su cui si trova, e nei test i campi non si possono cercare per etichetta. **Misura: 137 gruppi in 30 file** (i più carichi: `views/WebAssets/index.jsx` 20, `modules/clients/ui/ClientForm.jsx` 11, `views/Agency/AgencyProjectNewPage.jsx` 9, Quotes 24 in 5 file, Calendar 17 in 3 file, Projects 12 in 3 file). **Come si sta chiudendo:** *in scia ai giri di spezzatura dei file .jsx*, non con una passata unica — i file che si riscrivono si sistemano lì, con i test nuovi che dimostrano il collegamento (già fatto così in `modules/projects/ui/` durante i due giri pipeline, e in `views/Agency/project/web/WebPageSettingsCard.jsx` il 4/8). Restano da coprire le aree che il riordino .jsx non tocca (Quotes, Team, Settings, Authentication): **sessione dedicata**, da decidere. ✅ **Calendar chiuso il 5/8/2026** durante la spezzatura: i sette gruppi del modulo evento hanno il loro `controlId` e un test lo dimostra (`board/CalendarBoardPageUi.test.jsx`, *"tutti i campi si raggiungono dalla loro etichetta"*). Gli altri gruppi scollegati di quella cartella stanno tutti nei **quattro file morti** (`AddCategory`, `CreateNewEvent`, `EventsDrawer`, `SetReminder`): si chiudono cancellandoli, non correggendoli. ✅ **Campi personalizzati chiusi il 1/9/2026** (CRM-51, in scia all'estrazione della Sezione 7 di `ClientForm.jsx`): i campi della sezione e quelli della modale di creazione nascono con il loro `controlId`, e i test li cercano per etichetta. ⚠️ **I 12 gruppi propri di `ClientForm.jsx` (Sezioni 1-6) restano scollegati:** quel file non è stato riscritto, solo alleggerito, e la mappa CRM-42 parte D vieta di spezzarne altri blocchi in questo giro — si chiudono quando lo riapre chi lo spezza. ⚠️ **Nota che senza un controllo il buco si riapre:** il plugin che lo verifica (`eslint-plugin-jsx-a11y`, regola `label-has-associated-control`) **non è installato** — aggiungerlo a livello di avviso, in linea con gli altri guardrail, è una decisione a sé perché è una dipendenza nuova.
...
- **Dimensione dei file: il censimento completo e chi spezza cosa** *(deciso da Jacopo il 5/8/2026; è la fonte di verità a cui rimanda `CLAUDE.md`)*.
  ...
  **② Il residuo — nessuna V li nomina: vanno alla V13 (pulizia finale)**
  ...
  - **`scripts/security/rbac-usage.mjs` (484 righe al 1/9/2026)** — l'estrattore di permessi su cui poggia il controllo `npm run security:rbac-catalog`. **Non era in nessuna lista** fino al 1/9/2026: censito qui su rilievo della terza tappa di revisione di CRM-35 (CRM-67, punto 3), perché un file a quasi 500 righe che nessuna lista sorveglia è come ci si ritrova a 600 senza che nessuno l'abbia deciso. **Non va spezzato adesso**, ed è una scelta motivata e non un rinvio: delle 484 righe **296 sono codice**, 134 commento e 54 vuote, e **la crescita è stata quasi tutta commento** — sono i limiti noti dell'estrattore scritti per esteso, che in un file di questo tipo è esattamente ciò che si vuole. Spezzarlo costerebbe tre importatori nuovi in mezzo a un compito ancora aperto.
    **La riga di taglio è già individuata**, per chi ci tornerà: *lettore di sorgente* (`stripNonCode` e dintorni, righe 1-200 — è il pezzo che da solo compra di più); *catalogo e usi* (200-410); *rotte* (`ROUTE_*`, `collectGuardAliases`, `extractRoutePermissions`, da ~410, una sessantina di righe: da solo compra poco).
    ⚠️ **Da qui in avanti ogni aggiunta di LOGICA va nel file nuovo, non in coda a questo.** Le aggiunte di solo commento non violano la regola, ma spingono comunque verso la soglia: passate le 500 righe il taglio non è più rimandabile.
...
- **L'import dei clienti non controlla i doppioni** *(trovato l'1/9/2026 dal collaudatore su CRM-73, confermato leggendo il codice durante CRM-50)*. Importando **due volte lo stesso file** i clienti entrano **due volte**: `importClientsFromCsv` chiama `clientsRepository.create` per ogni riga valida senza mai cercare se quel cliente esista già (`server/modules/clients/service.ts`, il ciclo dentro `if (!dryRun)`), e non c'è nessun vincolo di unicità sul database che glielo impedisca. **È preesistente**, non l'ha introdotto l'anteprima; ma l'anteprima lo **rende visibile**, perché ora annuncia «1 riga entrerà» senza avvisare che quella riga c'è già — ed è esattamente il punto in cui una persona si aspetterebbe l'avviso. **Non è una riparazione, è lavoro nuovo**, e prima serve una decisione di prodotto che non spetta a chi passa di lì: *cosa rende due clienti "lo stesso cliente"?* (l'email? la partita IVA? il nome esatto?) e *cosa si fa quando succede* — si salta la riga, si aggiorna il cliente esistente, o si chiede all'utente riga per riga. Ogni risposta è un lavoro diverso: la prima costa poco, la seconda è un "import con aggiornamento" a tutti gli effetti. Il posto dove atterrerebbe è **l'anteprima**, che è già costruita per elencare le righe con un motivo accanto: basterebbe un terzo elenco («già presenti»). ⛔ **Non anticiparlo dentro un altro lavoro sui clienti.**

- **Gli errori dell'import dei clienti sono in inglese, e ora si leggono** *(trovato l'1/9/2026 dal collaudatore su CRM-73; misurato leggendo il codice durante CRM-50)*. La misura: in `server/modules/clients/service.ts` ci sono **38 messaggi d'errore, e 36 sono in inglese** — gli unici due in italiano sono quelli scritti per l'import ad allegato (CRM-48/50). Chi importa un'anagrafica sbagliata legge quindi cose come `name is required`, `email must be a valid email`, `type must be person or company` **accanto** a «Numero di telefono non valido» (che è italiano perché vive altrove, in `clients.schema.ts`). Due precisazioni che cambiano il lavoro:
  - **Non è una semplice traduzione.** I messaggi sono costruiti come `` `${fieldName} is required` `` con dentro il **nome tecnico del campo** (`name`, `type`, `address.country`, `cf:budget`): tradurre la frase senza tradurre il nome darebbe «name è obbligatorio», che è peggio di prima. Serve una mappa *nome tecnico → etichetta italiana*, che oggi non esiste, e serve tenerla allineata ai campi personalizzati (che sono definiti dall'utente).
  - **Due messaggi fermano l'import intero, non una riga:** `CSV header contains unknown columns` e `CSV header must include "name" column` (righe 509 e 516, sollevati da `assertCsvColumns` alla riga 982, cioè **prima** che venga letta qualunque riga). Basta una colonna scritta male perché l'anteprima si chiuda con una frase in inglese e nessun elenco. Sono i due da tradurre per primi, se si vuole spendere poco.
  **Preesistente** — l'anteprima non li ha creati, li ha portati in primo piano: prima l'esito mostrava al massimo cinque errori dopo aver già salvato, ora sono il contenuto principale della schermata che si legge *prima* di decidere. Va con la **fase A del re-naming** (le etichette che l'utente legge), voce «Re-naming delle aree» qui sopra.

- **Gli 11 colori scritti a mano di `clients-ui.css`** *(misurato l'1/9/2026 durante CRM-50, terzo debito del collaudo dell'import clienti)*. `npm run lint:css` segnala **11 avvisi, tutti nello stesso file**: `src/modules/clients/ui/clients-ui.css` (1.160 righe), alle righe **351-353, 372-374, 378, 390-391, 680-681**. Sono colori fissi (`#hex` e `rgba(...)`) in regole normali, quindi la **deroga `@media print` non c'entra**: in tutto il file non esiste nessun blocco di stampa. Cinque selettori, e in tema scuro non si comportano tutti allo stesso modo:
  - `.clients-avatar` (righe 351-353) — testo `#1f2937`, gradiente `#dbeafe`/`#bfdbfe`, bordo interno `rgba(59, 130, 246, 0.25)`. **A schermo questo è a posto**, perché più su, alla riga **303**, c'è già un `[data-bs-theme="dark"] .clients-avatar` che rifà le stesse tre proprietà con i token (`--hk-bg-secondary`, `--hk-border-secondary`, `--hk-text-primary`). È debito di forma, non un difetto visibile — e insieme la **traccia di come si ripara**: quei token sono la traduzione già scritta, basta portarla nella regola base e cancellare l'override.
  - `.clients-avatar-icon` (372-374, sfondo `#fff`, bordo `rgba(0, 0, 0, 0.08)`, icona `#1d4ed8`), `.clients-empty-state .card-body` (378, bordo tratteggiato `rgba(100, 116, 139, 0.35)`), `.clients-empty-icon` (390-391, `#334155` su `#e2e8f0`) e `.clients-copy-field-icon` (680-681, `#334155` su `rgba(71, 85, 105, 0.11)`) — **questi no: non hanno nessun override scuro**, quindi in tema scuro restano chiari. Il pallino bianco sull'avatar, il tondo dell'elenco vuoto e l'icona del campo da copiare sono macchie chiare su fondo scuro. Qui il difetto si vede.
  **Preesistente**: non l'ha introdotto il ramo dell'anteprima import (CRM-50), il cui blocco CSS nuovo comincia a riga 1080 e usa solo token — con e senza quel ramo gli avvisi sono gli stessi 11, alle stesse righe. ⚠️ **Perché nessuno l'ha raccolto prima, ed è la parte da ricordare:** in `.stylelintrc.json` la regola ha `"severity": "warning"`, quindi **`npm run lint:css` stampa gli 11 avvisi e poi esce con codice 0** — è verde. Non è un cancello, è un promemoria che scorre via; ed è anche il motivo per cui la voce sul test di login qui sopra può dire in buona fede che «`lint:css` funziona» (era ed è vero nel senso dell'esito, non in quello del contenuto). ⛔ **Non correggerlo di passaggio mentre si lavora ad altro sui clienti**: è la classica cosa trovata per strada, e vale la regola generale. Lo raccoglie **la prossima V che mette mano all'interfaccia Clienti**, come primo passo prima di aggiungerci roba; se nessuna ci arriva, va alla **V13 (pulizia finale)**.
