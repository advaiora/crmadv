...
- **Fase 4 — Tempo reale (websocket)** ⏸️ **MESSA DA PARTE — decisione utente.** Tre nodi: (1) nuova dipendenza `@fastify/websocket`; (2) **auth del WS**: app solo-Bearer, il browser non manda header custom → token nell'URL, che **urta la regola privacy** del progetto; (3) non collaudabile live ora. Alternativa più economica ma che contraddice la spec: **polling anche sulla chat AI** (come la messaggistica). *Da scegliere insieme.*
- **Fase 3b — Immagini "con vista"** ⛔ **BLOCCATA**: storage binari = migrazione + `prisma generate` (lock DLL dell'altra sessione) + motore multimodale non collaudabile senza chiavi.
- **Fase 3b — Estendere allegati a Fonti/Preventivi + thread messaggistica** 🟡 **DA DECIDERE (UX)**: il server già li accetta come snapshot testo, ma la semantica *apri-vs-allega* per entità non-ambito e il punto d'ingresso UI sono da concordare (dettaglio e raccomandazioni nella spec, Fase 3b).
