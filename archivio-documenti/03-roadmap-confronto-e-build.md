...
- **Dimensione dei file: il censimento completo e chi spezza cosa** *(deciso da Jacopo il 5/8/2026; è la fonte di verità a cui rimanda `CLAUDE.md`)*.
  ...
  **③ Fuori perimetro — non si toccano, e non è una dimenticanza**
  ...
  > ⚠️ **Stessa cosa, secondo caso: `server/modules/clients/service.ts` è passato da 1.283 a 1.388 righe il 9/9/2026** (CRMA-58, la metà server dei quattro campi cliente — PEC, codice destinatario SDI, sito web, referente). Era **già** oltre la soglia-mostro, quindi qui non c'è una soglia superata: c'è un file difficile che diventa più difficile. Anche qui l'estrazione **è** stata fatta — le regole di forma dei campi stanno in `server/modules/clients/field-rules.ts` (89 righe, col suo test), e in `service.ts` non è entrata **nessuna funzione nuova**: solo voci in elenchi che esistevano già (`CLIENT_BODY_FIELDS`, `CSV_HEADER_COLUMNS`, la mappa degli alias, `mapClient`, i due parser del corpo, il registro attività). Sono **sei elenchi paralleli** che vanno tenuti allineati a mano: è quella ripetizione, non la lunghezza, il vero costo del file.
  > **Quando si spezza:** il file è fra «i 22 file backend sopra 500 righe» qui sopra, cioè **V13**, e nessuna V della release lo riapre. Le righe di taglio sono già visibili a occhio: **l'import/export CSV** (intestazioni, alias, conversione riga↔corpo, export — da solo vale più di metà file) e **la validazione del corpo** (i due parser e i loro normalizzatori). Chi ci tornerà tolga il CSV per primo.
...
- **Gli errori dell'import dei clienti sono in inglese, e ora si leggono** *(trovato l'1/9/2026 dal collaudatore su CRM-73; misurato leggendo il codice durante CRM-50)*. La misura: in `server/modules/clients/service.ts` ci sono **38 messaggi d'errore, e 36 sono in inglese** — gli unici due in italiano sono quelli scritti per l'import ad allegato (CRM-48/50). *(Aggiornamento 9/9/2026, CRMA-58: i messaggi sono ora **38 in inglese su due file** — 35 in `service.ts` più 3 in `server/modules/clients/field-rules.ts`, dove sono finite le regole di forma dei quattro campi nuovi insieme al controllo dell'email, che da `service.ts` se n'è andato. I due italiani restano dov'erano. Chi tradurrà deve aprire tutti e due i file.)* Chi importa un'anagrafica sbagliata legge quindi cose come `name is required`, `email must be a valid email`, `type must be person or company` **accanto** a «Numero di telefono non valido» (che è italiano perché vive altrove, in `clients.schema.ts`). Due precisazioni che cambiano il lavoro:
