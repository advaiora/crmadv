...
> **Da fare più avanti (rifattorizzazione Console piattaforma):** oggi la Console piattaforma è **un'unica pagina** (`PlatformConsole.jsx`) con tutto dentro (workspace + admin + costi AI + config AI): rischia di diventare troppo densa e poco intuitiva. **Separarla in due moduli/sotto-pagine sotto la tab "Console":** (1) **Gestione workspace & accessi** (provisioning workspace, sospensione, config AI per workspace, Super Admin di piattaforma); (2) **Consumi & costi AI**, da **arricchire** con il **tracciamento per singolo utente** (non solo per workspace) + **filtri** (per utente, periodo, modello, funzione) per monitorare il consumo di ogni utente. Nota tecnica: `AiUsageLog` oggi salva `workspaceId` ma **non** l'`userId` della chiamata → per il tracciamento per-utente andrà aggiunto `userId` al modello (nuova migrazione) e propagato l'id utente fino a `runAgencyOpenAiJsonWithMeta`.
