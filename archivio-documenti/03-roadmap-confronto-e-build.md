...
- **Fase 3b — Byte veri di tutti gli allegati + immagini** ✅ **FATTA (16/7)** — nuova tabella `AiConversationAttachmentBinary` + **migrazione `20260716152454_ai_attachment_binary`** (⚠️ **da segnalare a Claudio**, dopo l'arretrato `20260706085001`); upload salva i byte (le **immagini** prima rifiutate ora si allegano), download dell'originale dal chip. Verificato col round-trip byte sul DB reale. ⏳ Resta la **"vista" multimodale** (far *vedere* l'immagine al modello): solo con le **chiavi**, a fine V.
- **Fase 3b — Allegare Fonti/Preventivi + thread messaggistica** 🟢 **DECISO (16/7), DA COSTRUIRE (#3)**: per le entità non-ambito **solo il selettore della chat di destinazione** (Opzione B, pieno controllo utente); la stessa *"scegli la chat"* va aggiunta come **terza opzione a tutti gli allegati**. Thread messaggistica dalla `MessagingPanel` (solo i propri). Dettaglio nella spec, Fase 3b.
- **Selettore del modello AI in chat** 🟡 **DA FARE / DA DECIDERE INSIEME (dal 17/7)** *(chiesto da Jacopo il 16/7)*. Aggiungere in Chat AI un selettore per **scegliere il modello** da usare. **Profondità da decidere insieme:** (a) solo **provider** (OpenAI vs Anthropic), oppure (b) **provider + modello specifico** di ciascuno (Anthropic: Opus/Sonnet/Haiku; OpenAI: gpt-5/gpt-4o…). Da chiarire anche l'**ambito della scelta** (per messaggio / per sessione / override del default di workspace già in Impostazioni Agency) e che siano selezionabili **solo i provider con chiave** configurata. Il motore multi-provider esiste già (V5): serve la UI + passare il modello scelto a `runAgencyAiTextWithMeta`. **Nessuna decisione presa: vediamolo insieme.**
- **Regola anti-blocco "altra sessione con API accesa"** 🟡 **DA CONCORDARE** *(chiesto da Jacopo il 16/7)*. Definire una **regola di progetto** che eviti strutturalmente la situazione in cui un'altra sessione tiene su l'API (`tsx watch`) e blocca `prisma generate`/migrazioni (lock DLL) e mostra dati vuoti al reload. Contesto e workaround già in **nota operativa #28**; qui va **decisa la regola** (es. una sola sessione avvia i dev server; convenzione su chi/quando li tiene; o un accorgimento tecnico). Da concordare, **non ancora scritta in CLAUDE.md**.
- **Sez. 4-ter punto 5 — Non letti / notifiche nel popup** 🟡 *(opzionale, da decidere)*. La messaggistica ha già i non letti; la **chat AI no** — inventarlo è lavoro nuovo.
