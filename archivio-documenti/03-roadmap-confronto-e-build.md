...
- **Sez. 4-ter punto 4 — Modulo Messaggi = la casella** (16/7): `/apps/email` è ora la chat a tutto schermo montata su rotta, con lo stesso selettore dei due mondi. **"Una sola implementazione"**: modalità `inline` su `AiChatWidget` (stesso componente, non una copia), corpo condiviso in una `const surface`. Nuova `src/views/Messaging/MessagingInboxPage.jsx`; vecchia `Email/index.jsx` sostituita e rimossa; voce/rotta/badge non-letti invariati. *Resa visiva da eyeballare: pane di anteprima bloccata sulle pagine pesanti (nota operativa 27); tutti i controlli offline verdi + console pulita.*
