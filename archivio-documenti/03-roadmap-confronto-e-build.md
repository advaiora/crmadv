...
- ⚠️ **In `package.json` esiste un blocco `overrides`, e chi tocca le dipendenze deve saperlo** *(9/9/2026, CRMA-64)*. Sono due righe — `uuid: ^11.1.1` e `deepmerge-ts: ^8.0.0` — che **forzano la versione di due pacchetti che nessuno di noi importa**: ci arrivano dentro `exceljs` e dentro `prisma`. Servono a chiudere due avvisi di sicurezza per i quali `npm audit fix --force` proponeva l'unica altra strada disponibile, cioè **retrocedere** exceljs da 4.4.0 a 3.4.0 e Prisma da 6.19.3 a 6.12.0. Il perché sono lì, e il quando si tolgono:
  - **`uuid`** — exceljs 4.4.0 dipende da `uuid@8.3.2`, che ha un difetto di controllo dei limiti del buffer. exceljs usa solo `v4()`, che con uuid 11 si comporta identico: verificato scrivendo un `.xlsx` e rileggendolo con i due lettori veri del CRM (`parseExcelBuffer` dell'area Agency e `readClientImportFile` dell'import Clienti). **Si toglie** quando exceljs pubblicherà una versione che dipende da uuid ≥ 11.1.1 (oggi l'ultima stabile è la 4.4.0, ferma da un pezzo).
  - **`deepmerge-ts`** — la catena è `prisma` → `@prisma/config` → `deepmerge-ts` (esaurimento dello stack su grafi ricorsivi). **Si toglie** quando `@prisma/config` salirà a `deepmerge-ts` 8.
  - ⚠️ **La trappola da conoscere:** un `overrides` è invisibile finché non fa danno. Se un giorno exceljs o Prisma smettono di funzionare dopo un aggiornamento e il nostro codice non è cambiato, **si guarda lì prima di ogni altra cosa**. E prima di aggiungerne uno nuovo si rilegge questa voce: la regola seguita è *forzare la dipendenza colpevole, mai retrocedere il pacchetto che la ospita*.

- **`npm audit fix` alza la CLI di Prisma e lascia indietro il client** *(9/9/2026, CRMA-64)*. È successo davvero: `prisma` (devDependency) è salito a 6.19.3 mentre `@prisma/client` (dependency) è rimasto a 6.16.2. Generatore e runtime disallineati sono uno stato che Prisma non sostiene, e non danno nessun errore in faccia: si vedono solo come tipi che non tornano più. **Dopo ogni `npm audit fix`, controllare che le due versioni coincidano** e riallinearle prima di committare il lock. ⚠️ Riallineandole emerge un errore di tipo che dalla 6.16 era latente: dalla 6.19 un campo `Bytes` si dichiara `Uint8Array<ArrayBuffer>` e il `Buffer` di `@fastify/multipart` non lo soddisfa più. È già chiuso in `server/repositories/ai-conversation.repository.ts` con una vista che non copia i byte — **se ne ricompare uno uguale altrove, il modello è quello**.
