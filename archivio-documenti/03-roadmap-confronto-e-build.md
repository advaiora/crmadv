...
> 3. **`AgencyProjectAdsPage.jsx:304` — probabile refuso:** `campaignGoal: projectItem.goal ? current.output.campaignGoal : current.output.campaignGoal` restituisce lo stesso valore sui due rami, quindi l'obiettivo campagna **non viene mai aggiornato** quando si usa una campagna come contesto corrente; la riga sotto (`offerAngle`) fa invece la cosa analoga correttamente. Da verificare e sistemare qui, con l'intento originale sotto mano.
