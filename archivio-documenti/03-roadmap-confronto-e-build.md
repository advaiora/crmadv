...
| **Team** | ~~L'invito è meccanicamente corretto ma **la consegna è rotta**: senza server di posta l'email non parte e il CRM dice comunque "successo". Da fare: configurare la posta (`noreply@advaiora.com`), maschera di configurazione dentro il CRM, esito di consegna veritiero, **pulsante "copia link invito"** (è ciò che rende il flusso a prova di guasto). Più: modifica membro oggi è una funzione vuota, filtro "In attesa" morto~~ → ✅ **CORREZIONE 31/8/2026: risulta costruito quasi per intero** (commit `b989f95`, `030c90e`, `184f283`). Maschera «Server di posta» con una configurazione sola per tutti i lettori SMTP, esito di consegna veritiero, «copia link invito» con rigenerazione, modifica membro e filtro "In attesa". 🟡 **Resta aperto un pezzo solo: la password della casella nel `.env`**, che nessun agent può verificare — la mette Jacopo o Claudio sulla macchina. ⛔ Il traguardo **non è dichiarato chiuso**: è una decisione del consiglio. Verifica di **sola lettura del codice, non a schermo**; dettaglio e limiti nel piano, §7.3 ④ |
