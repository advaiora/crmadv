...
- **Primitiva:** `CollapsibleSection` (`src/components/ui/CollapsibleSection.jsx`) — contenitore animato riusabile. Lo stato aperto è controllato dal chiamante (es. un `Set` di id per multi-open).
  - ⚠️ **Corretto il 25/8/2026.** Qui c'era scritto *«altezza misurata in JS e animata con `transition: height`»*: **è il contrario di quello che fa il codice**, che lo dichiara nella propria intestazione — *«Animazione basata su TRANSFORM (compositore/GPU), non su `height`»*. Come funziona davvero: **l'altezza viene misurata in JS e assegnata in un colpo solo** (un reflow), e ciò che si anima è il contenuto interno che scorre con **`translateY`**. Il motivo è prestazionale: animare l'altezza rifarebbe il layout a ogni frame, e su questo template pesante costringerebbe a ri-raster dei `backdrop-filter` (il blur della barra). **Il compromesso, che va conosciuto:** i vicini «saltano» alla posizione finale invece di crescere gradualmente, perché un `transform` non sposta il layout. Da chiuso il contenuto è reso `inert`.
  - *Divergenza trovata dallo sviluppo delle skill Paperclip (`crm-design-frontend`) e verificata sul codice. Se copi questo meccanismo in un componente nuovo, leggi prima la regola `ui-collapse-animating` in `globals.css`: non è stata riletta in questa passata.*
