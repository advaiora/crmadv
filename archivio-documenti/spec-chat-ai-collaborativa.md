...
### Fase 3b — Immagini "con vista" + thread di messaggistica ⏳ PARZIALE (16/7): storage byte + immagini FATTI; resta multimodale (chiavi), Fonti/Preventivi UI e thread messaggistica
...
- **Estendere "Chiedi all'AI" a Fonti/Preventivi**: 🟡 **DA DECIDERE (UX).** Il **server accetta già** gli snapshot testuali di `source` e `quote` (`ATTACHABLE_ENTITY_TYPES` in `chat-attachments.ts`): manca solo abilitarli lato UI. Ma una **fonte non è un ambito** (non c'è "apri chat su una fonte"), quindi va ridefinita la semantica del menu contestuale (**solo "aggiungi alla chat aperta"** per queste entità) e va gestito l'**allega-senza-chat-aperta** (sequenza async a rischio corsa, nota #24). *Raccomandazione: per le entità non-ambito mostrare solo "aggiungi", e se non c'è chat aperta aprire la **Generale** e allegare lì (il server risolve la sessione col `conversationId` omesso).* Default UX da confermare prima di costruire.
- **Thread di messaggistica come allegato**: 🟡 **DA DECIDERE (UX).** `WorkspaceMessage` è 1-a-1 **senza entità thread** — va identificato dalla coppia (mittente, destinatario), non da un id. Nota privacy: solo i thread di cui l'utente fa parte. Server pulito (aggiungere `messaging_thread` a `ATTACHABLE_ENTITY_TYPES` + snapshot dei messaggi con il peer, nessuna migrazione perché `entityType` è una stringa). **Manca il punto d'ingresso UI** (da dove si allega un thread? dalla `MessagingPanel`?) — decisione.
...
### Estensione — selettore del modello AI in chat 🟡 DA DECIDERE INSIEME (chiesto da Jacopo il 16/7)
- Aggiungere in **Chat AI** un selettore per **scegliere il modello** con cui rispondere. **Profondità da decidere insieme:** (a) solo **provider** (OpenAI vs Anthropic), oppure (b) **provider + modello specifico** per ciascuno (Anthropic: Opus/Sonnet/Haiku; OpenAI: gpt-5/gpt-4o…).
- **Da chiarire anche:** l'**ambito della scelta** — per singolo messaggio, per sessione, o override del **default di workspace** già configurato in Impostazioni Agency; e che siano proposti **solo i provider con una chiave** configurata (gli altri disabilitati).
...
- **Stato:** nessuna decisione presa, **niente costruito**. Prima discussione: scegliere la profondità (a/b) e l'ambito.
...
5. **Non letti / notifiche**: 🟡 **DA DECIDERE (opzionale, non fatto).** Liberi di integrarli nel popup. Nota: `WorkspaceMessage` ha già `readAt` (la messaggistica **mostra già** i non letti nell'elenco persone); la **chat AI non ha alcun concetto di "non letto"** — inventarlo è lavoro nuovo, **da valutare, non darlo per scontato**. Un badge sul pulsante in topbar guidato dai soli non letti della **messaggistica** (che il dato ce l'ha) sarebbe la fetta economica, ma aggiunge un poll sulla topbar: da concordare.
