...
### Fase 3b — Immagini "con vista" + thread di messaggistica ⏸️ PARZIALE (16/7): widget fatto, resto bloccato/da decidere

> **Decisioni del 16/7/2026 (Jacopo) — registrate, da realizzare in ordine dopo la Fase 4:**
> - **Storage dei binari esteso a TUTTI gli allegati file.** Non più *"estrai il testo e butta il file"*: si **conservano i byte veri** di ogni allegato (immagini incluse), **in aggiunta** al testo estratto che l'AI legge / il RAG indicizza (quel flusso non si tocca). ⚠️ *Reverte la scelta documentata "il binario non viene conservato" (Fase 3a) e diverge dal modulo **Fonti**, che oggi scarta il binario: in fase di build va confermato se allineare anche le Fonti, più nota su crescita dello storage.* Richiede **migrazione DB** (a DB libero) + `prisma generate`. Sblocca anche le **immagini** (ostacolo A); resta l'ostacolo B (motore multimodale) collaudabile solo con le **chiavi**, a fine V.
...
- **Immagini**: ⛔ **BLOCCATA.** Richiede (a) **storage dei binari** — una **tabella/colonna dedicata** → una **migrazione + `prisma generate`**, oggi impossibile senza fermare l'API dell'altra sessione (lock DLL, nota operativa #15); il precedente è `uploads/agency-sources/` del vecchio codice Agency; e (b) un **motore AI multimodale** — `runAgencyAiTextWithMeta` accetta `content: string`, va esteso a blocchi su **tutti e tre** i rami (Anthropic `/v1/messages`, OpenAI `/v1/responses`, fallback `chat/completions`), **non collaudabile senza chiavi**. Da fare quando si può migrare **e** ci sono le chiavi.
