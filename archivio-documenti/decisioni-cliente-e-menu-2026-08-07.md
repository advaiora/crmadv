...
2. **Cambio password** (⑥-ter n.1). È il blocco più grave trovato: senza, chi dimentica la password non rientra più. Subito dopo la posta, perché il recupero la usa. ⚠️ **Qui dentro va chiuso anche il punto 7 del §7.7** («Prova connessione» che raggiunge la rete interna): da quando esiste il recupero password, chi ha `mail.manage` può dirottare le email di reset di chiunque. Non prima, non dopo.
...
7. **📌 DA DECIDERE — «Prova connessione» del Server di posta può raggiungere indirizzi della rete interna** *(sollevato dal revisore il 18/8/2026, spostato qui dalla roadmap generale il 18/8/2026: e' lavoro della release, non debito trasversale)*. Il pulsante apre una connessione TCP verso l'host e la porta scritti nella maschera, e riporta a schermo il messaggio d'errore vero (*connection refused* / *timeout* / *handshake*). Chi ha il permesso `mail.manage` può quindi usarlo per capire cosa risponde su `10.0.0.5:6379`: è una scansione della rete interna con un oracolo che risponde. **Perché non è stato chiuso subito:** il rimedio ovvio — riusare `isBlockedHostname`/`isPrivateIpv4Address` di `server/core/net-guard.ts`, già scritti per le scansioni SEO — **romperebbe un caso legittimo**, cioè un server di posta dentro la rete dell'agenzia, che è tutt'altro che raro. Va scelto: (a) lasciare com'è, accettando che il permesso sia dato solo a chi amministra davvero (oggi Superadmin e Admin); (b) bloccare gli indirizzi privati con un interruttore che permetta di autorizzarli quando servono; (c) tenere aperto ma restituire un errore generico invece di quello vero, perdendo però la diagnosi che è il motivo per cui il pulsante esiste. **Nel frattempo la prova scrive nel registro attività** (`mail.test`, con esito, origine e server), quindi l'uso resta tracciato. Peso reale della cosa: basso finché il CRM gira per una sola agenzia, cresce il giorno che diventa multi-azienda con amministratori che non sono noi.

   ⚠️ **Va deciso PRIMA del recupero password, non dopo** *(osservazione del revisore, 18/8/2026)*. Il recupero password è il punto ② dell'ordine di lavorazione della release: dal momento in cui esiste, chi ha `mail.manage` può **dirottare le email di reset di chiunque** — Superadmin compreso — puntando il server di posta su una casella propria. Oggi il permesso vale "chi manda gli inviti"; da allora vale "chi può prendersi qualunque account del workspace". Non è un motivo per bloccare il recupero password, è un motivo per chiudere questa voce mentre lo si costruisce.
...
9. **📌 DA DECIDERE — «Prova connessione» collauda i parametri SALVATI, non quelli scritti a schermo** *(sollevato dal revisore il 18/8/2026; Jacopo: da sistemare, non è comportamento voluto)*. Chi modifica un campo della maschera e preme *Prova connessione* senza salvare ottiene un esito che parla della **configurazione precedente**. Il messaggio nomina sempre il server provato — quindi chi legge con attenzione se ne accorge — ma l'avviso *«Salva le impostazioni per poter provare la connessione»* compare **solo quando non c'è ancora niente di salvato**, non quando ci sono modifiche non salvate. Il caso concreto in cui morde: si corregge la porta da 587 a 465, si preme Prova, esce *«connessione riuscita»* riferito alla porta vecchia, e si conclude che la modifica è buona senza averla mai collaudata. **Due strade:** (a) disabilitare *Prova connessione* finché ci sono modifiche non salvate, con l'etichetta che spiega perché — semplice, ma costringe a salvare una configurazione che magari non funziona; (b) lasciarlo attivo e mostrare un avviso *«stai provando la configurazione salvata, non quella che hai appena scritto»* quando i campi risultano modificati — più fedele a com'è fatta la pagina, perché la prova gira sul server e il server usa ciò che è salvato. **Va fatto insieme al punto 1 dell'ordine di lavorazione** (Server di posta), non a sé: è la stessa maschera.
