...
| **Team** | Invito **meccanicamente corretto** (token, scadenza, accettazione, creazione membership). ~~**Rotto nella consegna**: senza server di posta configurato l'email non parte, ma l'API risponde comunque "creato con successo" e l'interfaccia lo conferma. Nessun "reinvia", nessun modo di recuperare il link dopo. Inoltre: modifica membro è **una funzione vuota** dichiarata "fase 2"; filtro "In attesa" nell'elenco membri è **morto** (non può dare risultati).~~ → ✅ **CORREZIONE 31/8/2026: questa fotografia è del 17/8 ed è stata superata dal lavoro fatto il 17 e il 18/8.** La consegna dell'invito dice la verità, il link si copia e si rigenera, e le due lacune (modifica membro, filtro "In attesa") risultano costruite — commit `b989f95`, `030c90e`, `184f283`. **Verifica di sola lettura del codice, non a schermo:** il dettaglio riga per riga, e quello che resta aperto, stanno in ④. |
...
> ✅ **CORREZIONE 31/8/2026 — quasi tutto questo punto risulta GIÀ COSTRUITO.** Il piano è del 17/8 e descrive cinque lavori da fare; alla verifica del repository (commit `ac25489`) quattro su cinque risultano fatti fra il 17 e il 18/8, con i commit `b989f95` (*«l'invito dice la verita', e la posta ha una configurazione sola»*), `030c90e` (la pagina «Server di posta») e `184f283` (*«nessuno invita a un ruolo piu' alto del proprio»*). **I punti restano scritti come erano, con la loro storia, e sono annotati uno per uno qui sotto** — non si cancella il ragionamento, si dice cosa è successo dopo.
>
> ⚠️ **Limite dichiarato della verifica: è stata di sola lettura del codice, non a schermo.** Dice che il codice c'è e che è collegato, non che si comporta bene aprendo le pagine. Dove sotto è scritto *«risulta costruito»* va letto così: la certezza a schermo è un giro di collaudo, non altra lettura.
>
> ⛔ **Il traguardo 1 dell'ordine di lavorazione (§7.5) NON è dichiarato chiuso**, e non lo può dichiarare chi scrive qui: resta aperto il punto 1 (la password della casella nel `.env`), che nessuna verifica sul repository può toccare — quel file è escluso apposta. Chiudere o no il traguardo è una decisione sul perimetro, quindi **è del consiglio**.
...
1. **Configurare il server di posta.** ✅ Dati forniti da Jacopo il 17/8: mittente **`noreply@advaiora.com`** *(confermato: `noreplay` era un refuso)*, host `mail.advaiora.com`, porta `587`. 🔐 **La password NON è in questo documento e non deve mai finirci**: va solo nel file `.env`, che è escluso dal repository. Poiché è transitata in chat, valutare di cambiarla dopo la configurazione.
   - 🟡 **APERTO al 31/8/2026 — è l'unico punto di ④ che resta.** L'impianto attorno c'è: le migrazioni `20260818100621_mail_server_settings` e `20260818101500_mail_permissions` sono in `prisma/migrations/`, e il permesso `mail.manage` è nel catalogo (`server/auth/rbac-catalog.ts:53`). **Ma la password della casella sta nel `.env`, fuori dal repository: nessun agent può verificare che ci sia, né metterla.** Lo chiude **Jacopo o Claudio sulla macchina**; finché non risulta fatto la posta non parte davvero, quindi non è un residuo di rifinitura — è il pezzo che rende utile tutto il resto di ④.
2. **Maschera di configurazione dentro il CRM** ✅ (richiesta di Jacopo): i parametri del server di posta devono essere modificabili dalle impostazioni, non solo da file.
   ...
   - ✅ **RISULTA COSTRUITO (verificato il 31/8/2026).** La maschera è `src/views/Settings/MailServer.jsx`, col suo test accanto (`MailServer.test.jsx`), e dietro c'è il modulo `server/modules/mail/` completo (servizio, repository, cifratura, criteri, rotte). **Le tre implementazioni indipendenti sono diventate una sola:** l'invito (`server/modules/team/team-invite.notifier.ts`) e i preventivi (`server/modules/quotes/notifications.ts`) leggono entrambi `resolveMailTransportDettagliato` da `server/core/mail.ts`, e **il terzo lettore morto `src/core/email/mailer.ts` non esiste più** (zero file tracciati sotto `src/core/email`). Commit `b989f95` per l'unificazione, `030c90e` per la pagina.
3. ~~**Dire la verità nell'interfaccia:** la risposta deve distinguere "invito creato **e** email partita" da "invito creato, **email non partita**", e la schermata deve mostrarlo invece di dire sempre "successo".~~
   - ✅ **RISULTA COSTRUITO (verificato il 31/8/2026).** Stesso commit `b989f95`, che nel titolo dice esattamente questo: *«l'invito dice la verita'»*. L'esito di consegna passa per `src/modules/team/ui/inviteDelivery.js` (`describeInviteDelivery`), usato in `src/views/Team/index.jsx` sia alla creazione dell'invito sia nell'elenco.
4. ~~**Pulsante "copia link invito"** accanto a ogni invito in attesa. È ciò che rende il flusso a prova di guasto: se la posta non funziona, il link si manda a mano.~~
   - ✅ **RISULTA COSTRUITO (verificato il 31/8/2026).** Il pulsante «Copia link» c'è **due volte** in `src/views/Team/index.jsx` (righe 550 e 680): una subito dopo la creazione dell'invito, una nell'elenco degli inviti — e con esse un flusso di **rigenerazione** del link. *(Sulla rigenerazione restano valide due annotazioni già scritte in ⑤: la rotta non ha limite di frequenza, e ogni chiamata invalida il link precedente.)*
5. ~~**Chiudere le due lacune** ✅ approvate: la modifica membro oggi vuota, e il filtro "In attesa" morto.~~
   - ✅ **RISULTANO COSTRUITE (verificato il 31/8/2026), con la prudenza del limite dichiarato sopra.** **Modifica membro:** `src/views/Team/index.jsx:310` conferma con *«Ruolo aggiornato»* e alla riga 706 c'è la maschera dei ruoli — la funzione non è più vuota. **Filtro "In attesa":** `src/views/Team/index.jsx:199` filtra gli inviti per stato, e `PENDING` è usato in più punti (righe 562-576) — il filtro non è più morto. Il codice c'è ed è collegato; che si comportino bene lo dice solo un giro a schermo.
   - 📌 **Non è però tutto il giro delle due lacune.** Restano due cose già scritte altrove e ancora aperte: **(a)** `revokeInvite` e `deleteInvite` non hanno nessun controllo di gerarchia (⑤, «Resta scoperto, da valutare fuori da questo giro»), che quel giro doveva chiudere «con una regola sola»; **(b)** la spezzatura di `src/views/Team/index.jsx`, che la roadmap assegna proprio a questo giro perché il file è a ridosso della soglia-mostro. Chi riprende il Team guardi lì prima di cominciare.
...
🔐 **CHIUSA IL 17/8/2026 — una scala di privilegi nell'invito** *(trovata dal revisore lavorando al punto ④, chiusa con la regola dettata da Jacopo lo stesso giorno)*.

**Il difetto:** in tutto il CRM i ruoli di sistema li assegna **solo un Superadmin** (`assertActorCanAssignRole`, applicato sia al cambio ruolo sia alla creazione diretta di un membro). L'invito era l'unico punto fuori riga: accettava **qualunque** preset, Superadmin compreso, da chiunque avesse `team.invite`. Un Manager poteva crearsi un invito da Superadmin, aprirlo, ed entrare con pieni poteri. Restava teorico solo perché senza posta configurata il link non usciva mai — il pulsante "Link invito" del punto ④ lo avrebbe reso praticabile.

✅ **La regola, dettata da Jacopo:** *«un ruolo più basso non deve mai essere in grado di creare un invito per un ruolo più alto di lui»*. Vale universalmente, senza casi particolari. **Stesso livello è concesso** (un Admin può invitare un Admin); si applica sia alla creazione dell'invito sia alla consegna del link di uno già esistente, perché sono lo stesso potere. La scala usata è quella che il workspace ha già (`SYSTEM_ROLE_PRIORITY`: Superadmin 50 › Admin 40 › Manager 30 › Operativo 20 › Viewer 10): non se ne è introdotta una seconda, che prima o poi divergerebbe. A schermo la tendina mostra solo i ruoli concedibili, il server rifiuta comunque.

✅ **Insieme alla gerarchia resta in vigore un secondo divieto: il preset Superadmin non si concede per invito, nemmeno da un altro Superadmin.** Sembra ridondante e non lo è, perché **promuovere e invitare non sono lo stesso potere**: promuovere agisce su un membro che *esiste già*, con un account e una password; invitare produce **una stringa al portatore** — la rotta di accettazione è pubblica, l'autenticazione facoltativa, e senza sessione l'invito *crea* l'utente su quell'email e restituisce un token d'accesso. Da oggi quel link viene mostrato e copiato negli appunti quando l'email non parte, quindi finisce in chat e nei messaggi: un invito da Superadmin sarebbe un oggetto per cui **chi lo apre per primo comanda**, destinatario o no. Il resto del progetto la pensa già così — la registrazione declassa a Viewer chi chiede Superadmin. *(Questo divieto era stato tolto per un paio d'ore il 17/8 con il ragionamento "tanto un Superadmin può già promuovere chiunque": ragionamento sbagliato, rimesso dopo la seconda revisione.)*

⚠️ **Terzo controllo, meno ovvio: invitare di nuovo qualcuno SOVRASCRIVE l'invito che aveva.** Cambia ruolo e token, quindi il link già consegnato smette di funzionare. È un potere sull'invito *di qualcun altro*, e va misurato sul ruolo di **quell'**invito, non solo su quello nuovo — altrimenti un Manager può calpestare l'invito da Admin appena spedito dal Superadmin, declassarlo e prendersi il link. Il controllo c'è.

*Scartate lungo la strada:* **(a)** lasciare com'era, cioè un Manager può invitare un Admin — invitare e promuovere sarebbero due poteri diversi, ma è proprio l'asimmetria che apriva la falla; **(c)** solo il Superadmin invita — coerente, ma renderebbe l'invito inutile proprio nella release che serve a far entrare le persone.

📌 **Resta scoperto, da valutare fuori da questo giro:** `revokeInvite` e `deleteInvite` **non hanno nessun controllo di gerarchia**. Un Manager non può *prendersi* il link di un invito da Admin, ma può ancora **revocarlo o cancellarlo**. Non è una scala di privilegi (non guadagna poteri), è un dispetto possibile fra colleghi; e la stessa domanda vale per il futuro pulsante "reinvia". Da chiudere insieme, con una regola sola, quando si affrontano le due lacune del Team.
...
1. **Server di posta + invito Team** (④). Sblocca l'inserimento di persone vere, serve a collaudare tutto il resto con più utenti, ed **è il prerequisito** del punto 2 se si sceglie il recupero password via email.
   - ⚠️ **CORREZIONE 31/8/2026 — questo traguardo NON è da fare da zero: risulta costruito quasi per intero.** La verifica del repository al commit `ac25489` trova fatti quattro dei cinque punti di ④ (maschera «Server di posta» e configurazione unica, esito di consegna veritiero, «copia link invito» con rigenerazione, le due lacune del Team) — commit `b989f95`, `030c90e`, `184f283`. **Il dettaglio, coi limiti della verifica, è in ④: si legge lì.** Chi riprende non deve ricostruire niente di tutto questo.
   - 🟡 **Resta aperto un solo pezzo, ed è ④.1: la password della casella nel `.env`.** Non è verificabile da nessun agent (il file è fuori dal repository) e **lo chiude Jacopo o Claudio sulla macchina**. Finché non risulta fatto, il traguardo non è concluso e il punto 2 (cambio e recupero password) non ha il suo prerequisito.
   - ⛔ **Perciò il traguardo resta scritto come aperto, e non lo si dichiara chiuso qui.** Dichiararlo sarebbe una decisione sul perimetro della release, che **spetta al consiglio** — non a chi aggiorna il documento. **La sequenza delle undici voci non è stata toccata:** cambia cosa risulta già costruito dentro il punto 1, non il suo posto in fila.
2. **Cambio password** (⑥-ter n.1). È il blocco più grave trovato: senza, chi dimentica la password non rientra più. Subito dopo la posta, perché il recupero la usa. ⚠️ **Qui dentro va chiuso anche il punto 7 del §7.7** («Prova connessione» che raggiunge la rete interna): da quando esiste il recupero password, chi ha `mail.manage` può dirottare le email di reset di chiunque. Non prima, non dopo. ✅ **Vincolo assolto in anticipo:** il guardiano della rete privata era già su `main` prima che il recupero password nascesse — vedi §7.7 punto 7, ✅ CHIUSO il 9/9/2026.
...
1. ✅ **Messaggi e Profilo: verificati il 17/8** — vedi ⑥-bis e ⑥-ter. Il perimetro non è più incerto. Le tre decisioni che restavano sono state prese — vedi §7.10. ⚠️ I punti 7 e 8 qui sotto NON sono quelle: sono due voci arrivate il 18/8, spostate qui dalla roadmap generale.
...
7. ✅ **CHIUSO — «Prova connessione» del Server di posta può raggiungere indirizzi della rete interna** *(sollevato dal revisore il 18/8/2026, spostato qui dalla roadmap generale il 18/8/2026; chiuso il 9/9/2026)*. Il pulsante apre una connessione TCP verso l'host e la porta scritti nella maschera, e riporta a schermo il messaggio d'errore vero (*connection refused* / *timeout* / *handshake*). Chi ha il permesso `mail.manage` può quindi usarlo per capire cosa risponde su `10.0.0.5:6379`: è una scansione della rete interna con un oracolo che risponde. **Perché non è stato chiuso subito:** il rimedio ovvio — riusare `isBlockedHostname`/`isPrivateIpv4Address` di `server/core/net-guard.ts`, già scritti per le scansioni SEO — **romperebbe un caso legittimo**, cioè un server di posta dentro la rete dell'agenzia, che è tutt'altro che raro. Le tre strade valutate: (a) lasciare com'è, accettando che il permesso sia dato solo a chi amministra davvero (oggi Superadmin e Admin); **(b) bloccare gli indirizzi privati con un interruttore che permetta di autorizzarli quando servono** — la strada scelta; (c) tenere aperto ma restituire un errore generico invece di quello vero, perdendo però la diagnosi che è il motivo per cui il pulsante esiste. **Nel frattempo la prova scrive nel registro attività** (`mail.test`, con esito, origine e server), quindi l'uso resta tracciato.

   **Costruito con la strada (b):** `server/modules/mail/mail.net-guard.ts` esporta `richiedeControlloRetePrivata` (vero solo per la configurazione `source: 'database'`, e solo se l'interruttore `retePrivataConsentita` non è attivo), insieme ai due messaggi di rifiuto `ERRORE_RETE_PRIVATA` / `ERRORE_RETE_PRIVATA_ALLA_CONNESSIONE` e a `messaggioDaNascondere` (toglie l'indirizzo interno dal messaggio d'errore di nodemailer, un secondo filtro perché il DNS può risolvere diversamente fra il controllo e la connessione vera). Provato in `server/modules/mail/mail.net-guard.test.ts`: `richiedeControlloRetePrivata(daDatabase(false)) === true`, `richiedeControlloRetePrivata(daDatabase(true)) === false`. Unito a `main` con i merge `46e8d4f` (ramo `crm-28-prova-connessione-rete-privata`) e `4699268` (ramo `crm-56-interruttore-rete-interna`, l'interruttore a schermo).

   ⚠️ **Andava deciso PRIMA del recupero password, non dopo** *(osservazione del revisore, 18/8/2026)* — **soddisfatto in anticipo**: il guardiano della rete privata esisteva già su `main` quando il recupero password è nato (punto 2 dell'ordine di lavorazione sopra), quindi il vincolo non ha mai trovato un varco aperto. Il recupero password è il punto ② dell'ordine di lavorazione della release: dal momento in cui esiste, chi ha `mail.manage` può **dirottare le email di reset di chiunque** — Superadmin compreso — puntando il server di posta su una casella propria. Oggi il permesso vale "chi manda gli inviti"; da allora vale "chi può prendersi qualunque account del workspace". Non è un motivo per bloccare il recupero password, è un motivo per chiudere questa voce mentre lo si costruisce — ed è stato chiuso prima, non dopo.

8. **⚠️ Il primo login dopo l'avvio dell'API può rispondere 500, e ogni permesso nuovo avvicina il momento in cui capiterà sempre** *(misurato il 18/8/2026 lavorando alla pagina «Server di posta»; spostato qui dalla roadmap generale il 18/8/2026)*. Al login gira `ensureWorkspaceAccessDefaults` (`server/routes/auth.route.ts:562`), che dentro **una sola transazione Prisma da 5 secondi** fa un `upsert` alla volta per ogni modulo del catalogo, ogni permesso e ognuno dei cinque ruoli di sistema. **A freddo la transazione ha impiegato 5015 ms contro un tetto di 5000** e il login è morto con un `P2028` — *"Transaction already closed"* — che l'utente vede come *Errore interno del server*. A caldo lo stesso login costa 1,3-1,5 s, quindi il problema si vede **solo al primo accesso dopo un riavvio**: cioè esattamente il primo accesso dopo un rilascio. **Perché sta qui e non è stato risolto sul momento:** non l'ha causato il lavoro sulla posta (il tetto era già sfiorato), ma quel lavoro ha aggiunto un modulo e un permesso alla stessa transazione, e la release di settembre ne aggiungerà altri (cestino, recupero password, allegati). Il margine si assottiglia a ogni giro. **Tre strade, in ordine di costo:** (a) alzare il `timeout` della transazione interattiva — una riga, ma sposta il problema; (b) togliere la sincronizzazione del catalogo dalla transazione del login e farla all'avvio del server, dove nessuno aspetta; (c) sostituire i cicli di `upsert` uno-a-uno con inserimenti massivi idempotenti (`createMany` + `skipDuplicates`), che è la causa vera della lentezza. **La (b) è quella che toglie il problema invece di rimandarlo**, ma cambia quando il catalogo si allinea, quindi va decisa e non fatta di straforo.

9. ✅ **CHIUSO — «Prova connessione» collauda i parametri SALVATI, non quelli scritti a schermo** *(sollevato dal revisore il 18/8/2026; Jacopo: da sistemare, non è comportamento voluto; chiuso il 9/9/2026)*. Chi modifica un campo della maschera e preme *Prova connessione* senza salvare ottiene un esito che parla della **configurazione precedente**. Il messaggio nomina sempre il server provato — quindi chi legge con attenzione se ne accorge — ma l'avviso *«Salva le impostazioni per poter provare la connessione»* compare **solo quando non c'è ancora niente di salvato**, non quando ci sono modifiche non salvate. Il caso concreto in cui morde: si corregge la porta da 587 a 465, si preme Prova, esce *«connessione riuscita»* riferito alla porta vecchia, e si conclude che la modifica è buona senza averla mai collaudata. Le due strade valutate: (a) disabilitare *Prova connessione* finché ci sono modifiche non salvate, con l'etichetta che spiega perché — semplice, ma costringe a salvare una configurazione che magari non funziona; **(b) lasciarlo attivo e mostrare un avviso quando i campi risultano modificati** — la strada scelta, più fedele a com'è fatta la pagina, perché la prova gira sul server e il server usa ciò che è salvato.

   **Costruito con la strada (b):** `src/views/Settings/mailServerModifiche.js` è il rilevamento delle modifiche non salvate (`CAMPI_VUOTI`, `campiDaImpostazioni`, e il confronto generico per chiave su cui si appoggia l'avviso), usato da `src/views/Settings/MailServer.jsx` per mostrare l'avviso quando i campi a schermo divergono da quelli salvati. Unito a `main` con il merge `6053066` (ramo `crm-27-avviso-prova-configurazione-salvata`), che porta con sé anche i test (`MailServer.test.jsx`, `mailServerModifiche.test.js`).
