...
> **Dal 9/9/2026 lo stesso controllo si fa sulle schede Paperclip.** I mestieri del team esistono in **due sistemi con gli stessi nomi** — i tre agent di repository descritti qui e le schede agente di Paperclip. Una scheda a zero attività va letta come una quota subagent a zero: **condizione di innesco mai scattata, non mestiere inutile.** Quale dei due sistemi si usa e quando sta in `CLAUDE.md`, sezione *«La regola mista vale per tutta la squadra»*.
...
| 24/8/2026 | **Decisione 1 di Paperclip sciolta: l'ambiente di sviluppo del CRM vive tutto sulla VPS** — database PostgreSQL, API (4000) e Vite (5173). Gli agent possono quindi far girare l'intera suite e vedere il CRM davvero. ⚠️ **Due conseguenze:** la macchina di partenza diventa il **KVM 4** (16 GB) e non più il KVM 1, perché con l'ambiente sulla macchina i 4 GB non bastano; e il «lucchetto di Prisma» temuto nel piano **non si applica su Linux** (era un comportamento di Windows) — resta invece la contesa vera, due agent che migrano lo stesso database insieme, da verificare in fase 0-2. Dettaglio in `piano-paperclip-2026-08-19.md` §12.4. | Jacopo |
...
| 9/9/2026 | **La regola mista si estende a tutta la squadra.** Jacopo: *«gli agenti di Paperclip servono al 100%, nessuna rimozione»*. Sei schede su tredici erano a zero attività **non** perché il mestiere fosse inutile, ma perché nessuna condizione le svegliava mai. ⚠️ **Il testo che vale è la tabella di `CLAUDE.md`** (§«La regola mista vale per tutta la squadra»): quello che segue è il riassunto di cosa è stato deciso, non le condizioni da applicare — se i due divergono, si corregge questa riga. Per ogni mestiere è scritto lì il momento preciso in cui si paga il giro di compito Paperclip invece di risolvere dentro la sessione: mappa destinata a un altro agente → **Esploratore**; codice di un punto di roadmap o di release → **Sviluppatore backend / Sviluppatore frontend** (due compiti legati quando tocca `server/**` *e* `src/**`); schema, permessi, sicurezza, unioni a `main` → **Revisore**; qualunque cosa si veda a schermo → **Collaudatore** (mestiere *senza* gemello di repository); diff che tocca i cinque innesti AI → **Collaudatore AI**; permesso o rotta nuovi → **Guardiano**; più di tre compiti aperti insieme, o un compito tornato indietro → **Capocantiere**; note operative e roadmap → **Cronista**; squadra e costi → **Capo del personale**. **Scartate** «solo repository» (non lascia nessuna traccia che l'utente possa controllare da sé) e «solo Paperclip» (un giro di compito per ogni tappa: la catena si ferma). *Summarizer* e *Reflection Coach* restano in pausa: sono agenti di serie, fuori dai dieci mestieri del 19/8. Il **CEO** è l'unica scheda senza riga, ed è voluto — è chi legge la tabella e apre i compiti agli altri; la sua regola è che **non tiene per sé il lavoro che la tabella assegna a qualcun altro**, nemmeno quando farlo in sessione sarebbe più rapido. ⚠️ **Conseguenza sul §2 di questo documento:** il campanello d'allarme «quota subagent vicina a zero = condizioni di innesco scritte male, non agent inutili» vale ora identico per le schede Paperclip a zero attività. | Jacopo |
