...
## 70. Tredici compiti fermi otto ore: un legame di blocco senza motivo, uno stato confuso con la colonna, un assegnatario mancante

**Contesto:** notte fra il 9 e il 10/9/2026, release di settembre. Tredici compiti della catena sono rimasti fermi per otto ore senza che nessuno se ne accorgesse in tempo.

**Errore:** tre cause distinte, sommate. **Primo:** una catena di dieci compiti legati da `blockedBy` (`28 → 45 → 29 → 46 → 30 → 31 → 32 → 47 → 33 → 34 → 23`), senza una sola diramazione — cioe' una quarantina di run in sequenza e mai due agenti al lavoro insieme — dove alcuni legami non nominavano nessun motivo tecnico: erano solo un ordine preferito, scambiato per un blocco reale. **Secondo:** nessuno dei tredici compiti era in stato `blocked`, erano gia' tutti in `todo`: spostarli di colonna non poteva avere nessun effetto, perche' il fermo stava nel grafo dei bloccanti, non nella colonna. **Terzo:** un compito `critical` e' rimasto fermo quindici ore su un segreto trapelato perche' non aveva nessun assegnatario, quindi nessuno lo vedeva.

**Modo corretto:**
- **Un legame `blockedBy` e' tecnico o non esiste.** Motivi ammessi, da nominare per esteso nel compito bloccato: stesso file riscritto in profondita' (si nomina il file), stessa tabella o stessa migrazione (si nomina), un dato o una funzione che il secondo consuma e che non esiste finche' il primo non e' chiuso (si nomina). *«Prima questo poi quello»* non e' un motivo: quello e' l'**ordine**, e si esprime con la **priorita'**, non con un blocco. Senza il motivo nominato, il legame va tolto. **Tetto duro: nessuna catena piu' lunga di tre anelli.**
- **Il fermo si misura sull'attivita', non sulla colonna.** Il criterio meccanico da passare a ogni giro: attivita' piu' vecchia di 2 ore senza run attivo → si rilancia con un commento; `blocked` senza bloccanti → si rimette in `todo`; fuori da `backlog` senza assegnatario → si assegna; tutti i bloccanti chiusi ma il compito resta fermo → si rilancia. **Un risveglio differito non si ritenta da solo:** se `claimedAt` e `runId` restano nulli, quella revisione non e' lenta, non e' mai iniziata.
- **Nessun compito esce da `backlog` senza assegnatario.** Un compito senza assegnatario e' invisibile a chiunque non lo cerchi apposta, a prescindere dalla priorita' scritta sopra.
- Le regole sui cancelli da mettere su un compito Paperclip (quando serve il Guardiano, quando il Revisore, quando nessuno) sono scritte in `CLAUDE.md`, sezione «Team di agent» → «Quali cancelli mettere su un compito Paperclip».
